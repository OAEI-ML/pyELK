...
| `allow_incomplete_imports` | `False` | Required to accept a view whose core provenance records an ignored or unresolved import closure; every result then carries the `PYELK_IGNORED_IMPORT` policy reason. Without it, construction raises `pyowl_core.UnresolvedImportError`. |
...
reason. An invalid `PYELK_BACKEND` or `PYELK_PURE_PYTHON` value is reported
through `selection_error` rather than raised.
...
`fallback_reason` when `auto` selected Python. On encoded-native sessions,
`backend.compiler_handoff` additionally exposes the frozen encoded-compiler
attestation (schema name and version, model schema, descriptor SHA-256, and buffer
widths); on scalar sessions accessing it raises `AttributeError`.
...
rebuild them. In particular, constructing a reasoner over a view with an
incomplete import closure without `allow_incomplete_imports=True` raises
`pyowl_core.UnresolvedImportError`. pyELK's own hierarchy, in `pyelk.exceptions`,
applies at the compiler/reasoner boundary:
...
`pyelk.exceptions` also defines `ParseError` and `UnresolvedImportError` in the
same hierarchy, but the installed facade does not raise them: parsing and
import-closure failures surface as the corresponding `pyowl_core` exceptions.
