...
Integrate parser, compiler, completeness, pure saturation, taxonomy, realisation, and queries
behind the final typed `Ontology`/`Reasoner` API. Implement raw-result validation, canonical
...
`src/pyelk/ontology.py` remains WP2-owned; this WP re-exports/uses it without restructuring.

## Forbidden paths

OWL/parser/indexing/reasoning implementations, completeness logic, contracts/codec, Rust,
build/release files, oracle/frozen fixture contents, benchmarks.

## Deliverables

1. Exact public exports, `ReasonerConfig`, result/node/taxonomy/instance values, re-exports of
   WP0 completeness contract identities, and lifecycle.
...
6. Import handling, complete `CompiledQuery.fresh_entities` validation, strict
...
4. Fresh allowed/disallowed, unresolved/ignored imports, unsupported ignore/error, unsupported
